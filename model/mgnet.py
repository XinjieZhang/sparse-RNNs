...
class Model:
    def __init__(self,
        ...
        self.y = tf.placeholder(tf.float32, [None, None, n_output])
        ...
        self.h, states = tf.nn.dynamic_rnn(cell, self.x, dtype=tf.float32, time_major=False)
        ...
        with tf.variable_scope("output"):
            ...
            w_out = tf.get_variable(
                'output_kernel',
                [n_hidden, n_output*hp['label_width']],
                dtype=tf.float32,
                trainable=True,
            ...
            b_out = tf.get_variable(
                'output_bias',
                [n_output*hp['label_width']],
                dtype=tf.float32,
                trainable=True,
                initializer=self.output_bias_ini
            )

            self.y_shaped = tf.reshape(self.y, (-1, n_output))
            self.pred = tf.matmul(self.h[:, -1, :], w_out) + b_out
            self.cost = tf.reduce_mean(
                tf.square(self.y_shaped - tf.reshape(self.pred, (-1, n_output))))
